...
std::vector<Players> InitManchesterUInfield;
std::vector<Players> ManchesterUInfield
{
...
std::vector<Players> ArsenalSubs
{
    ...
    {"Hector Bellerín",24,"Spain",178,74,2,"SUB","GOOD",17500000,92,51,70,78,76,66,20,20,20,20,80},
    {"S. Kolašinac",26,"Bosnia Herzegovina",183,85,31,"SUB","GOOD",13500000,75,62,72,73,75,87,20,20,20,20,79},
...
*       Liverpool
*/
Squad Liverpool("Liverpool F.C", "Premier League", "J�rgen Klopp", 11, 7);
...
Squad Chelsea("Chelsea F.C", "Premier League", "Frank Lampard", 11, 7);

std::vector<Players> InitChelseaInfield;
std::vector<Players> ChelseaInfield
{
    {"N. KantÃ©",	28,	 "France",	168,	72,	7,	 "RCM",	"GOOD",	66000000,	78,	65,	77,	81,	87,	83,	20,	20,	20,	20,	89},		
...
std::vector<Players> InitChelseaSubs;
std::vector<Players> ChelseaSubs
{
    {"M. KovaÄiÄ‡",	25,	 "Croatia",	176,	78,	17,	 "SUB",	"GOOD",	29000000,	76,	68,	81,	86,	67,	70,	20,	20,	20,	20,	82},		
    {"A. RÃ¼diger",	26,	 "Germany",	190,	85,	2,	 "SUB",	"GOOD",	24000000,	72,	43,	67,	63,	82,	78,	20,	20,	20,	20,	82},		
...
Squad Tottenham("Tottenham Hotspur F.C", "Premier League", "José Mourinho", 11, 7);

std::vector<Players> InitTottenhamInfield;
...
std::vector<Players> InitTottenhamSubs;
...
Squad CrystalPalace("Crystal Palace F.C.", "Premier League", "Roy Hodgson", 11, 7);

std::vector<Players> InitCrystalInfield;
...
std::vector<Players> InitCrystalSubs;
...
*       Everton
*/
Squad Everton("Everton F.C.", "Premier League", "Carlo Ancelotti", 11, 7);

std::vector<Players> InitEvertonInfield;
std::vector<Players> EvertonInfield
{
    ...
    {"G. SigurÃ°sson",	29,	 "Iceland",	186,	77,	10,	 "CAM",	"GOOD",	25500000,	56,	81,	84,	80,	57,	69,	20,	20,	20,	20,	83},		
    {"AndrÃ© Gomes",	25,	 "Portugal",	188,	84,	21,	 "LDM",	"GOOD",	17500000,	65,	71,	78,	80,	73,	80,	20,	20,	20,	20,	80},		
...
std::vector<Players> InitEvertonSubs;
std::vector<Players> EvertonSubs
{
    {"A. Iwobi",	23,	 "Nigeria",	185,	75,	17,	 "SUB",	"GOOD",	17000000,	80,	64,	74,	83,	28,	68,	20,	20,	20,	20,	79},		
    {"D. SidibÃ©",	26,	 "France",	182,	71,	19,	 "SUB",	"GOOD",	9000000,	77,	62,	74,	74,	73,	79,	20,	20,	20,	20,	77},		
...
*       Manchester City
*/
Squad ManchesterC("Manchester City", "Premier League", "Pep Guardiola", 11, 7);

std::vector<Players> InitManchesterCInfield;
std::vector<Players> ManchesterCInfield
{
    {"K. De Bruyne",	28,	 "Belgium",	181,	70,	17,	 "RCM",	"GOOD",	90000000,	76,	86,	92,	86,	61,	78,	20,	20,	20,	20,	91},		 
    {"S. AgÃ¼ero",	31,	 "Argentina",	173,	70,	10,	 "ST",	"GOOD",	60000000,	80,	90,	77,	88,	33,	74,	20,	20,	20,	20,	89},		 
...
std::vector<Players> InitManchesterCSubs;
std::vector<Players> ManchesterCSubs
{
    {"L. SanÃ©",	23,	 "Germany",	183,	75,	19,	 "SUB",	"GOOD",	61000000,	95,	81,	79,	86,	38,	70,	20,	20,	20,	20,	86},		 
    {"Rodri",	23,	 "Spain",	191,	82,	16,	 "SUB",	"GOOD",	47000000,	67,	68,	77,	77,	82,	80,	20,	20,	20,	20,	85},		 
    {"JoÃ£o Cancelo",	25,	 "Portugal",	182,	74,	27,	 "SUB",	"GOOD",	36500000,	90,	65,	80,	84,	78,	71,	20,	20,	20,	20,	84},		 
    {"I. GÃ¼ndoÄŸan",	28,	 "Germany",	180,	80,	8,	 "SUB",	"GOOD",	31000000,	66,	74,	84,	84,	72,	71,	20,	20,	20,	20,	84},		 
...
*       Leicester City
*/
Squad Leicester("Leicester City", "Premier League", "Brendan Rodgers", 11, 7);

std::vector<Players> InitLeicesterInfield;
std::vector<Players> LeicesterInfield
{
    ...
    {"Ayoze PÃ©rez",	25,	 "Spain",	178,	72,	17,	 "RW",	"GOOD",	13000000,	76,	75,	67,	81,	54,	66,	20,	20,	20,	20,	78},		
    {"H. Choudhury",	21,	 "England",	178,	64,	20,	 "LCM",	"GOOD",	8000000,	70,	44,	61,	66,	76,	75,	20,	20,	20,	20,	74},		
    {"C. SÃ¶yÃ¼ncÃ¼",	23,	 "Turkey",	185,	82,	4,	 "LCB",	"GOOD",	7500000,	63,	45,	52,	59,	74,	74,	20,	20,	20,	20,	74}		
};

std::vector<Players> InitLeicesterSubs;
std::vector<Players> LeicesterSubs
{
    {"F. BenkoviÄ‡",	21,	 "Croatia",	194,	91,	16,	 "SUB",	"GOOD",	11000000,	64,	46,	56,	56,	75,	82,	20,	20,	20,	20,	76},		
...
*
*/
Squad Juventus("Juventus F.C.", "SERIE A", "", 11, 7);
...
Squad InterMilan("Inter Milan", "SERIE A", "", 11, 7);
...
Squad Napoli("S.S.C. Napoli", "SERIE A", "", 11, 7);
...
Squad Atlanta("Atalanta B.C.", "SERIE A", "", 11, 7);
...
Squad Milan("A.C. Milan", "SERIE A", "", 11, 7);
...
Squad Roma("A.S. Roma", "SERIE A", "", 11, 7);





Squad Fiorentina("ACF Fiorentina", "SERIE A", "", 11, 7);
